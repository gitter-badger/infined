...
gulp.task('default', function (cb) {
    runSequence('plugins', 'libraries', 'tsd', 'ts', cb);
});
...
gulp.task('tsd', function (cb) {
  ...
  exec("tsd reinstall", function (err, stdout, stderr) {
    ...
    // Rebuild the typings?/tsd.d.ts bundle reference file.
 ...
 */
gulp.task('ts:declare', function (cb) {
  var majorVersion = 0,
      minorVersion = 0,
      buildVersion = 0;
 ...
 */
gulp.task('ts:typescripts', ['ts:typescripts-read-me'], function (cb) {
 ...
 * This will only copy the files if the build scheme is not set to release.
 */
gulp.task('ts:typescripts-read-me', function (cb) {
 ...
 * output the JavaScript to www/javascripts/bundle.js. Compilation parameters is located
 ...
 */
gulp.task('ts', ['ts:declare', 'ts:typescripts'], function (cb) {
  exec("tsc -p typescripts", function (err, stdout, stderr) {
    console.log(stdout);
    console.log(stderr);
    cb(err);
  });
 ...
 * sub-tasks.
 */
gulp.task('clean', ['clean:node', 'clean:bower', 'clean:platforms', 'clean:plugins', 'clean:chrome', 'clean:libraries', 'clean:ts']);
...
/**
 * Removes the www/public/libraries directory.
 */
gulp.task('clean:libraries', function (cb) {
  del([
    'www/public/libraries'
  ], cb);
});
